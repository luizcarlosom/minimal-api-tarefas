...
app.MapGet("/tarefas", async (AppDbContext db) => await db.Tarefas.AsNoTracking().ToListAsync());
...
app.MapGet("/tarefas/concluida", async (AppDbContext db) =>
                                 await db.Tarefas.AsNoTracking().Where(t => t.IsConcluida).ToListAsync());
...
{
    var tarefaSearch = await db.Tarefas.FindAsync(id);

    if (tarefaSearch is Tarefa tarefa) { 
